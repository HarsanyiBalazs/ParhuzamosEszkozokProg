...
int main(void)
{
    ...
    for (i = 0; i < SAMPLE_SIZE; ++i) {
        host_buffer1[i] = rand() % 100;
        //printf("[%d] = %f, ", i, host_buffer1[i]);
        atlag +=host_buffer1[i];
    }

    atlag=atlag/SAMPLE_SIZE;
    ...
    clSetKernelArg(kernel, 1, sizeof(float), (void*)&atlag);
    clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&device_buffer2);
